# Jambhala: A Batteries-Included Plutus Starter Kit
...
2. **Enable flakes & configure binary cache**
  ...
  * Modify the file following the instructions below:
    ...
    # Step 2c (OPTIONAL): Avoid unwanted garbage collection with nix-direnv
...
4. **Set up direnv**
  ...
  * Install the [direnv extension](https://marketplace.visualstudio.com/items?itemName=Rubymaniac.vscode-direnv&ssr=false#review-details) for VS Code.

5. **Load development environment**
  * Open the project root directory in VS Code.
  * In the terminal, you will see a message
  * From the project root directory in your terminal, run `nix develop` to load the Nix environment.
...
* **Updating `source-repository-package` entries in `cabal.project`:**
  ...
  * A script called `pa-sha` has been provided to conveniently calculate the sha256 hash for the `plutus-apps` repository. It can be run with a specific revision hash like so:
      ```sh
      pa-sha 78b5eaf2e27d58be631cbf8dd0e99735c55dbcd1
      ```
  If no revision argument is provided, it returns the most recent revision and its sha256 hash:
      ```
        ...
        {
          "url": "https://github.com/input-output-hk/plutus-apps.git",
          "rev": "78b5eaf2e27d58be631cbf8dd0e99735c55dbcd1",
          ...
          "sha256": "1ni765ifnmwhx7gxxsar2v0hpc42nvrkpq2nj3wdwxagh0zv5l8q",
          ...
        }
      ```
  * You can now update the `source-repository-package` entry for `plutus-apps` in the `cabal.project` file, replacing the `tag` and `--sha256` values with the `rev` and `sha256` values shown in the ouput of `pa-sha` (without quotation marks):
      ```
        source-repository-package
          type: git
          location: https://github.com/input-output-hk/plutus-apps.git
          tag: 78b5eaf2e27d58be631cbf8dd0e99735c55dbcd1
          --sha256: 1ni765ifnmwhx7gxxsar2v0hpc42nvrkpq2nj3wdwxagh0zv5l8q
          ...
      ```
  * Before updating `plutus-apps`, you should  also add/update any other dependencies this package requires and calculate sha256 hashes for them. You can do this using the `nix-prefetch-git` utility, which has been provided with this project's Nix development environment.
...
## Usage
**Building and serving `plutus-apps` docs**
  * To build and serve docs for the specific revision of `plutus-apps` this project is using, open a new bash terminal from the project root directory and run the following command:

    ```sh
    build-and-serve-docs
    ```

    The script will get the `plutus-apps` revision hash from the `cabal.project` file, clone the `plutus-apps` repository and checkout this revision, then launch a `nix-shell` and serve the docs at `http://0.0.0.0:8002/`.
