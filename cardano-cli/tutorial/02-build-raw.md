...
out="assets/protocol-parameters.json"
...
The script sets the filepath/name where the `--out-file` option will save the parameters, then runs the `query protocol-parameters` command with this value and the network option.

Run the script to save the protocol parameters:

```sh
$ params
Protocol parameters saved to 'assets/protocol-parameters.json'
```

Inspect the parameters output at `assets/protocol-parameters.json`, particularly the `txFee` attributes at the bottom of the file:
...
We'll now use the `calculate-min-fee` command to manually calculate the transaction fee, providing the draft as argument to the `--tx-body-file` option, plus some additional input data required to determine the fee:

```sh
$ cardano-cli transaction calculate-min-fee \
--tx-body-file $TX_PATH/transfer-raw.draft \
--tx-in-count 1 \
--tx-out-count 2 \
--witness-count 1 \
...
We provide the number of input and output UTXOs to the `--tx-in-count` and `--tx-out-count` options, respectively. The `--witness-count` option is provided the number of keys that must sign the transaction (in this case, just 1 - Alice's). We provide the network option using our `NET` environment variable as usual, and attach the protocol-parameters file we saved in **Step 1**.

Running the command prints a fee quantity in Lovelace, for example:

```sh
174433 Lovelace
```

We'll save this fee to a temporary variable so we can reference it later:
